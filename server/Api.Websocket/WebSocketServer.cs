...
public static class CustomWebSocketServer
{
    public static WebApplication StartWsServer(this WebApplication app)
    {
        ...
        Action<IWebSocketConnection> config = ws =>
        {
            ...
            ws.OnClose = () => scopedServices.GetRequiredService<IConnectionRegistry>().UnregisterConnection(connection);
            ws.OnError = ex =>
            {
                ...
                scopedServices.GetRequiredService<IServiceLogic>().Broadcast(JsonSerializer.Serialize(problemDetails), ws.ConnectionInfo.Id);
            };

...
export class DeviceClient {
    ...
    adminWantsToClearData(): Promise<Devicelog[]> {
        let url_ = this.baseUrl + "/AdminWantsToClearData";
        url_ = url_.replace(/[?&]$/, "");
        ...
        return this.http.fetch(url_, options_).then((_response: Response) => {
            return this.processAdminWantsToClearData(_response);
        });
    }

    protected processAdminWantsToClearData(response: Response): Promise<Devicelog[]> {
        const status = response.status;
        let _headers: any = {}; if (response.headers && response.headers.forEach) { response.headers.forEach((v: any, k: any) => _headers[k] = v); };
        if (status === 200) {
            return response.text().then((_responseText) => {
